...
# symbols which are imported by "from proteindigest.command import *"
...
from proteindigest.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("proteindigest"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="proteindigest.config")
...
def bootstrap():
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="ProteinDigest %s" % version)
    ...
    _read_config(args)
    from proteindigest.model import bootstrap_model
...
def start():
    ...
    from proteindigest.controllers import Root
